...
export function toDictionary<T extends { id: number }>(
  list: T[]
): { [id: number]: T } {
 ...
 *
 * Criar uma variavel "Dictionary"
 * const varDictionary = this.toDictionaryString<T>(lista);
 *
 * Criar uma variavel que receberá a variavel de dicionário passando como chave valor o "variavel: string"
 * const var = varyDictionary[id:string];
 *
 * Vantagens de se usar dicionário em várias requisições em uma só vez, ele é mais rápido do que um array.find()
 * podendo ganhar milissegundos ou segundos ao completar a função
 *
...
export function toDictionaryString<T extends { id: string }>(
  list: T[]
): { [id: string]: T } {
